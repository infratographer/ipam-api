...
echo "Creating NATS account for ipam-api"
nsc add account -n IPAM -K ${OPERATOR_SIGNING_KEY_ID}
nsc edit account IPAM --sk generate --js-mem-storage -1 --js-disk-storage -1 --js-streams -1 --js-consumer -1
nsc describe account IPAM

export ACCOUNTS_SIGNING_KEY_ID=`nsc describe account IPAM -J | jq -r '.nats.signing_keys | first'`

echo "Creating NATS user for ipam-api"
